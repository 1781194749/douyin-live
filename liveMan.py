...
class DouyinLiveWebFetcher:
    ...
    def __init__(self, live_id,danmaku_message=None):
        """
        直播间弹幕抓取对象
        ...
        self.danmaku_message = danmaku_message
    ...
    def _sendWebSocket(self):
        ...
        # 将headers字典转换为请求头列表
        request_headers = [f"{key}: {value}" for key, value in headers.items()]
        self.ws = websocket.create_connection(wss,header=request_headers)
        try:
            # 构造弹幕信息
            message = {
                "type": "send",
                "data": {
                    "message": self.danmaku_message
                }
            }

            # 将信息转换为 JSON 格式
            self.ws.send(json.dumps(message))
            print(f"成功发送弹幕: {self.danmaku_message}")
